...
export const comment = (() => {
   ...
   */
  const onNullComment = () => {
    return `<div class="text-center p-4 my-2 bg-theme-auto rounded-4 shadow"><p class="fw-bold p-0 m-0" style="font-size: 0.95rem;">Let's share this invitation so that there are many comments.</p></div>`;
  };
  ...
  const remove = async (button) => {
    ...
    const likes = document.querySelector(
      `[onclick="undangan.comment.like.love(this)"][data-uuid="${id}"]`
    );
    likes.disabled = true;
  ...
  const update = async (button) => {
    ...
    const cancel = document.querySelector(
      `[onclick="undangan.comment.cancel('${id}')"]`
    );
    if (cancel) {
  ...
  const send = async (button) => {
    ...
    const gifCancel = document.getElementById(
      `gif-cancel-${id ? id : "default"}`
    );
    ...
    if (id) {
      ...
      document
        ...
        .insertAdjacentHTML(
          "beforeend",
          await card.renderInnerContent(response.data)
        );
  ...
  const cancel = async (id) => {
    ...
    const badge = document.getElementById(`badge-${id}`);
    const isChecklist =
      badge && owns.has(id) ? badge.classList.contains("text-success") : false;
  ...
  const edit = async (button) => {
    ...
    await request(HTTP_GET, "/api/comment/" + id)
      ...
      .then(async (res) => {
        ...
        const isGif =
          res.data.gif_url !== null && res.data.gif_url !== undefined;
        if (isGif) {
          await gif.remove(id);
        }
        ...
        document
          ...
          .insertAdjacentElement(
            "afterend",
            card.renderEdit(id, res.data.presence, isParent, isGif)
          );
        ...
        if (isGif) {
          gif.onOpen(id, () => {
            ...
            document
              .querySelector(
                `[onclick="undangan.comment.gif.back(this, '${id}')"]`
              )
              ?.remove();
          });
        ...
        formInner.setAttribute(
          "data-original",
          util.base64Encode(res.data.comment)
        );
        return res;
      });
