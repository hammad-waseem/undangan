...
export const gif = (() => {
      ...
      request(HTTP_GET, url)
        ...
        .then((r) =>
          r
            ...
            .then((b) =>
              c
                .put(url, new Response(b, { headers: new Headers(r.headers) }))
                .then(() => b)
            )
        )
        .catch((err) => {
          ...
          el.insertAdjacentHTML(
            ...
            `
                    <figure class="gif-figure m-0 position-relative">
                        <div onclick="undangan.comment.gif.click('${
                          ctx.uuid
                        }', '${id}', '${util.base64Encode(
              url
            )}')" class="gif-checklist position-absolute justify-content-center align-items-center top-0 end-0 bg-overlay-auto p-1 m-1 rounded-circle border shadow-sm z-1">
  ...
  const infinite = async (ctx) => {
    ...
    const scrollableHeight =
      (ctx.lists.scrollHeight - ctx.lists.clientHeight) * 0.9;

    if (
      ctx.lists.scrollTop > scrollableHeight &&
      ctx.lists.getAttribute("data-continue") === "true"
    ) {
  ...
  const search = async (ctx, input) => {
    ...
    await bootUp(ctx);
    render(ctx, ctx.query === null ? "/featured" : "/search", {
      q: ctx.query,
      limit: ctx.limit,
    });
  };
  ...
  const singleton = (uuid) => {
    if (!objectPool.has(uuid)) {
      ...
      document
        ...
        .addEventListener("input", (e) => deSearch(ctx, e.target));
    }
  ...
  const remove = async (uuid = null) => {
    if (uuid) {
      ...
      await Promise.all(
        Array.from(objectPool.keys()).map((k) =>
          waitLastRequest(objectPool.get(k))
        )
      );
      objectPool.clear();
      queue.clear();
